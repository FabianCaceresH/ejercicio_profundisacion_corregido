...
def contador_generala(lista):
    conteo = []
    for x in range(6):
        conteo.append(lista.count(x+1))
    return lista
def elegir(lista):
    num_maximo = 0
    numero_elegido = 0
    num_maximo = max(lista)
    numero_elegido = (lista.index(num_maximo)+1)
    return num_maximo,numero_elegido
...
if __name__ == '__main__':
    ...
    conteo= 1
for x in range(5):
    print(f"numero de tiros {x+1} con cantidad de dados = {numero_de_dados}")
    tirar = lista_aleatoria(numero_de_dados)
    print(f"los dados que han sido tirados son {tirar}")
    if numero_de_dados == 5:
        seleccionar = elegir(contador_generala(tirar))
        print(f"el numero elegido es {seleccionar[1]} que salio {seleccionar[0]} veces")
        numero_elegido = seleccionar[1]
        conteo = conteo [0]
        if conteo < 1:
            numero_de_dados -= conteo
    elif numero_de_dados < 5 and numero_de_dados > 0:
        conteo_elegido = contar(tirar,numero_elegido)
        if conteo_elegido > 0 :
            numero_de_dados -= conteo_elegido
...
if numero_de_dados == 0 :
    ...
    # Leer el enunciado con atención y consultar cualquier dudaz